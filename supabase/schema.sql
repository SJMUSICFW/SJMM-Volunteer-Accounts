...
create table public.admin_users (
  ...
  display_name text not null check (char_length(display_name) between 1 and 100),
  admin_role text not null default 'manager' check (admin_role in ('owner', 'manager')),
  created_at timestamptz not null default now()
);
...
create policy households_admin_select on public.households
for select to authenticated
using (
  exists (
    select 1 from public.admin_users au
...
create policy contact_methods_admin_select on public.contact_methods
for select to authenticated
using (
  exists (
    select 1 from public.admin_users au
...
comment on table public.admin_users is
  'Allowlist for Music Ministry administrators. Only database owners may add or remove administrators.';
