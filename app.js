...
let account = null, demoMode = false, createStep = 1;
...
$$('[data-demo]').forEach(b=>b.onclick=async()=>{if(b.dataset.demo==="forgot"){const email=$("#loginIdentity").value.trim();if(!email)return toast("Enter your email address first.");const r=await db.auth.resetPasswordForEmail(email,{redirectTo:window.location.origin});return toast(r.error?r.error.message:"Password recovery instructions were sent.")}toast("Contact the St. Jude Music Ministry office for assistance.")});
db.auth.getSession().then(({data})=>{if(data.session)loadAccount().catch(err=>toast(err.message))});
